/*
 * @file parser.rs
 * @purpose Core G-code file parser and kinematic path analyzer.
 * @author Ed Moffatt
 */
use std::fs::File;
...
pub fn parse_gcode_file_impl(path: String) -> Result<GCodeAnalysis, String> {
        ...
        if changed {
            ...
            if m_type == 2 || m_type == 3 {
                ...
                for p in arc_points {
                    ...
                    last_x = p[0]; last_y = p[1]; last_z = p[2];
                }
                last_is_rapid = false;
            } else {
                ...
                last_x = current_x; last_y = current_y; last_z = current_z;
    ...
    Ok(GCodeAnalysis {
        points, operations, bbox_min, bbox_max,
        total_dist_cut, total_dist_rapid, estimated_time_s,
        min_z: bbox_min[2], max_z: bbox_max[2],
        workpiece_min_z, workpiece_max_z,
        min_feedrate: if min_feedrate == f32::MAX { 0.0 } else { min_feedrate },
        max_feedrate, wcs, unit: detected_unit, comments: header_comments, raw_lines,
    })
}
