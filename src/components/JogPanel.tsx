...
export function JogPanel() {
  ...
  const handleJog = (x: number, y: number, z: number) => {
    // Determine axis components based on diagonal
    // FluidNC/GRBL Jogging: $J=G91 G21 X... Y... F...
    // We send relative moves.
    ...
    invoke('send_gcode', { command: cmd });
  };

  const handleHome = () => {
      invoke('send_gcode', { command: '$H' });
  };
    ...
    <div className="h-full flex flex-col p-4 gap-6 select-none overflow-y-auto">
      ...
      <div className="flex flex-col gap-2">
         ...
         <input 
            ...
            className="accent-[var(--accent-primary)] w-full"
         />
      </div>
